...
const Plantas  = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://192.168.1.10:3000/plantas?timestamp=${new Date().getTime()}`);
        if (Array.isArray(response.data) && response.data.length > 0) {
          setPlantas(response.data);
          setFilteredPlantas(response.data);
          console.log('Data fetched successfully');
        } else {
          Alert.alert("Os dados referentes as plantas não foram encontrados na base de dados");
        }
      } catch (error) {
